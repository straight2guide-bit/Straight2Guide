...
export const metadata: Metadata = {
  title: "Find a Guide in Asia",
  description:
    "Search verified local guides across China, Vietnam, Thailand, Indonesia, the Philippines, and India. Direct bookings, fair pricing, authentic experiences.",
};

const ASIA_COUNTRIES = ["China", "Vietnam", "Thailand", "Indonesia", "Philippines", "India"];

const loadSearchParams = createLoader(guideSearchParsers);

export default async function AsiaGuidesPage({
  searchParams,
}: {
  ...
  if (supabaseConfigured) {
    ...
    let regionQuery = supabase
      ...
      .select("id, name, slug")
      .eq("is_active", true)
      .or(ASIA_COUNTRIES.map((c) => `country.ilike.${c}`).join(","))
      .order("name");
    ...
    guides = await fetchGuides({
      ...
      countries: activeCountry ? [] : ASIA_COUNTRIES,
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {ASIA_COUNTRIES.map((country) => {
              ...
              return (
                <Link
                  key={country}
                  href={`/guides/asia?country=${encodeURIComponent(country.toLowerCase())}`}
                  className={`rounded-full border px-4 py-1.5 text-xs font-semibold transition-colors ${
                    isActive
                      ? "border-[#0E7A45] bg-[#0E7A45] text-white"
                      : "border-slate-200 bg-white text-slate-600 hover:border-[#0E7A45] hover:text-[#0E7A45]"
        ...
        <div className="mx-auto flex w-full max-w-7xl gap-6 px-4 py-2 md:px-8">
          ...
          <section className="min-w-0 flex-1">
            <Suspense fallback={<GuideResultsGrid guides={[]} loading country={filters.country} />}>
              <GuideResultsGrid guides={guides} country={filters.country || "Asia"} />
            </Suspense>
          </section>
        </div>
      </div>
