...
export const metadata: Metadata = {
  title: "Find a Guide in South America",
  description:
    "Search verified local guides across Brazil, Peru, Bolivia, Colombia, Argentina, and Chile. Direct bookings, fair pricing, authentic experiences.",
};

const SA_COUNTRIES = ["Brazil", "Peru", "Bolivia", "Colombia", "Argentina", "Chile"];

const loadSearchParams = createLoader(guideSearchParsers);

export default async function SouthAmericaGuidesPage({
  searchParams,
}: {
  ...
  if (supabaseConfigured) {
    ...
    let regionQuery = supabase
      ...
      .select("id, name, slug")
      .eq("is_active", true)
      .or(SA_COUNTRIES.map((c) => `country.ilike.${c}`).join(","))
      .order("name");
    ...
    guides = await fetchGuides({
      ...
      countries: activeCountry ? [] : SA_COUNTRIES,
      ...
      <div className="flex-1 bg-slate-50 pt-10">
        ...
        <div className="mx-auto max-w-7xl px-4 pb-5 md:px-8">
          <div className="flex flex-wrap gap-2">
            ...
            {SA_COUNTRIES.map((country) => {
              const isActive = activeCountry === country.toLowerCase();
              return (
                <Link
                  key={country}
                  href={`/guides/south-america?country=${encodeURIComponent(country.toLowerCase())}`}
                  className={`rounded-full border px-4 py-1.5 text-xs font-semibold transition-colors ${
                    isActive
                      ? "border-[#0E7A45] bg-[#0E7A45] text-white"
                      : "border-slate-200 bg-white text-slate-600 hover:border-[#0E7A45] hover:text-[#0E7A45]"
        ...
        <div className="mx-auto flex w-full max-w-7xl gap-6 px-4 py-2 md:px-8">
          ...
          <section className="min-w-0 flex-1">
            <Suspense fallback={<GuideResultsGrid guides={[]} loading country={filters.country} />}>
              <GuideResultsGrid guides={guides} country={filters.country || "South America"} />
            </Suspense>
          </section>
        </div>
      </div>
