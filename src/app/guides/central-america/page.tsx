...
export const metadata: Metadata = {
  title: "Find a Guide in Central America",
  description:
    "Search verified local guides across El Salvador, Costa Rica, Guatemala, Panama, Nicaragua, and Belize. Direct bookings, fair pricing, authentic experiences.",
};

const CA_COUNTRIES = ["El Salvador", "Costa Rica", "Guatemala", "Panama", "Nicaragua", "Belize"];

const loadSearchParams = createLoader(guideSearchParsers);

export default async function CentralAmericaGuidesPage({
  searchParams,
}: {
  ...
  if (supabaseConfigured) {
    ...
    let regionQuery = supabase
      ...
      .select("id, name, slug")
      .eq("is_active", true)
      .or(CA_COUNTRIES.map((c) => `country.ilike.${c}`).join(","))
      .order("name");
    ...
    guides = await fetchGuides({
      ...
      countries: activeCountry ? [] : CA_COUNTRIES,
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {CA_COUNTRIES.map((country) => {
              ...
              return (
                <Link
                  key={country}
                  href={`/guides/central-america?country=${encodeURIComponent(country.toLowerCase())}`}
                  className={`rounded-full border px-4 py-1.5 text-xs font-semibold transition-colors ${
                    isActive
                      ? "border-[#0E7A45] bg-[#0E7A45] text-white"
                      : "border-slate-200 bg-white text-slate-600 hover:border-[#0E7A45] hover:text-[#0E7A45]"
        ...
        <div className="mx-auto flex w-full max-w-7xl gap-6 px-4 py-2 md:px-8">
          ...
          <section className="min-w-0 flex-1">
            <Suspense fallback={<GuideResultsGrid guides={[]} loading country={filters.country} />}>
              <GuideResultsGrid guides={guides} country={filters.country || "Central America"} />
            </Suspense>
          </section>
        </div>
      </div>
