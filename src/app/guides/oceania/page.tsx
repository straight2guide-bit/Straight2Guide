...
export const metadata: Metadata = {
  title: "Find a Guide in Oceania",
  description:
    "Search verified local guides across Australia, New Zealand, Fiji, Papua New Guinea, and Vanuatu. Direct bookings, fair pricing, authentic experiences.",
};

const OCEANIA_COUNTRIES = ["Australia", "New Zealand", "Fiji", "Papua New Guinea", "Vanuatu"];

const loadSearchParams = createLoader(guideSearchParsers);

export default async function OceaniaGuidesPage({
  searchParams,
}: {
  ...
  if (supabaseConfigured) {
    ...
    let regionQuery = supabase
      ...
      .select("id, name, slug")
      .eq("is_active", true)
      .or(OCEANIA_COUNTRIES.map((c) => `country.ilike.${c}`).join(","))
      .order("name");
    ...
    guides = await fetchGuides({
      ...
      countries: activeCountry ? [] : OCEANIA_COUNTRIES,
      ...
      <div className="flex-1 bg-slate-50 pt-10">
        ...
        <div className="mx-auto max-w-7xl px-4 pb-5 md:px-8">
          <div className="flex flex-wrap gap-2">
            ...
            {OCEANIA_COUNTRIES.map((country) => {
              const isActive = activeCountry === country.toLowerCase();
              return (
                <Link
                  key={country}
                  href={`/guides/oceania?country=${encodeURIComponent(country.toLowerCase())}`}
                  className={`rounded-full border px-4 py-1.5 text-xs font-semibold transition-colors ${
                    isActive
                      ? "border-[#0E7A45] bg-[#0E7A45] text-white"
                      : "border-slate-200 bg-white text-slate-600 hover:border-[#0E7A45] hover:text-[#0E7A45]"
        ...
        <div className="mx-auto flex w-full max-w-7xl gap-6 px-4 py-2 md:px-8">
          ...
          <section className="min-w-0 flex-1">
            <Suspense fallback={<GuideResultsGrid guides={[]} loading country={filters.country} />}>
              <GuideResultsGrid guides={guides} country={filters.country || "Oceania"} />
            </Suspense>
          </section>
        </div>
      </div>
